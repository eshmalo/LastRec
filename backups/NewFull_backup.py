...
def generate_gl_detail_report(
    ...
    for gl_account, gl_detail in sorted(gl_line_details.items()):
        ...
        if total_cam_net > 0 and cam_net > 0:
            ...
            # Check if this GL account is specifically excluded from admin fee calculation
            is_admin_fee_excluded = False
            if admin_fee_exclusions_list and check_account_exclusion(gl_account, admin_fee_exclusions_list):
                is_admin_fee_excluded = True

            # Only apply admin fee if this account is not specifically excluded from admin fee
            if not is_admin_fee_excluded:
                # Admin fee is calculated on CAM net for each GL line
                admin_fee_amount = cam_net * admin_fee_percentage
        ...
        if has_override and totals['tenant_share_amount'] > 0:
            ...
            if totals['tenant_share_amount'] == 0:
                for _, gl_data in sorted(gl_line_details.items()):
                    ...
                    gl_admin_fee = gl_cam_net * admin_fee_percentage
    ...
    for _, admin_value in gl_line_details.items():
